...
for service in /usr/lib/systemd/system/*.service; do
    ...
    if grep -q "^Requires=.*umount.target" "${service}"; then
        sed -i "/^Requires=.*umount.target/s/ *umount.target//g" "${service}"
    fi

    # Disable BPF/cgroup firewalling
    if grep -q "^IPAddressDeny=" "${service}"; then
        sed -i "/^IPAddressDeny=/s/IPAddressDeny=/#IPAddressDeny=/g" "${service}"
    fi
done
